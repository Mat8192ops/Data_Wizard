...
class MyMplCanvas(FigureCanvas):
    def __init__(self, parent=None, company_list=['msft']):
        ...
        for company_symbol in company_list:
            try:
                comp_data = pd.read_sql_query(
                    'SELECT Date, Close FROM {} WHERE "company_name" = "{}"'.format(tab_name, company_symbol), con)
                """ print(comp_data) """
